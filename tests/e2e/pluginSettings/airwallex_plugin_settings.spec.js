...
test.describe('Airwallex Settings Tab', () => {
    ...
    test('Airwallex General Tab', async () => {
        ...
        await expect(page.getByRole('rowheader', { name: 'Temporary order status after' }).locator('label')).toBeVisible();
        await expect(page.getByRole('row', { name: 'Order state for pending' }).locator('label')).toBeVisible();
        await expect(page.getByRole('row', { name: 'Order state for authorized' }).locator('label')).toBeVisible();
        await expect(page.getByRole('row', { name: 'Cronjob interval Cronjob' }).locator('label')).toBeVisible();
        await expect(page.getByRole('rowheader', { name: 'Activate JS logging' }).locator('label')).toBeVisible();
        await expect(page.getByRole('rowheader', { name: 'Activate remote logging' }).locator('label')).toBeVisible();
        await expect(page.getByRole('row', { name: 'Payment form template Payment' }).locator('label')).toBeVisible();
        ...
        await page.locator('#airwallex-online-payments-gatewayairwallex_general_payment_page_template').selectOption('wordpress_page');
    ...
    test('Airwallex Card Tab', async () => {
        ...
        await expect(page.getByRole('rowheader', { name: 'Checkout form' }).locator('label')).toBeVisible();
        await expect(page.getByRole('row', { name: 'Statement descriptor' }).locator('label')).toBeVisible();
        await expect(page.getByRole('rowheader', { name: 'Capture immediately' }).locator('label')).toBeVisible();
        await expect(page.getByRole('rowheader', { name: 'Capture status' }).locator('label')).toBeVisible();

        await page.getByText('Enable Airwallex Card Payments').check();
        await page.getByLabel('Title').fill('Credit Card');
        await page.getByLabel('Description').fill('');
        await page.locator('#airwallex-online-payments-gatewayairwallex_card_checkout_form_type').selectOption('inline');
