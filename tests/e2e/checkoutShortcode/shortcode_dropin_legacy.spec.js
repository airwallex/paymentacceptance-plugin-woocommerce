...
import { WP_NORMAL_USER_EMAIL_FOR_DROP_IN, WP_NORMAL_USER_PASSWORD_FOR_DROP_IN, TEST_CARD, TEST_CARD_3DS_CHALLENGE } from '../Shared/constants';
...
test.describe('Drop in element - Shortcode checkout page - Legacy payment template', () => {
  test('Success transaction - Simple product - Card no 3DS', async ({ page }) => {
    ...
    await page.frameLocator('iframe[name="Airwallex dropIn element iframe"]').getByRole('button', {name: 'Pay'}).nth(1).click();
  ...
  test('Success transaction - Subscription product - Card 3DS challenge - Renew subscription', async ({ page }) => {
    ...
    await page.frameLocator('iframe[name="Airwallex dropIn element iframe"]').getByRole('button', {name: 'Proceed'}).click();
