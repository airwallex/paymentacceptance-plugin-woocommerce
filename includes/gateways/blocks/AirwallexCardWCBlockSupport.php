...
class AirwallexCardWCBlockSupport extends AirwallexWCBlockSupport
{
    ...
    {
        $this->settings = get_option('airwallex-online-payments-gatewayairwallex_card_settings', []);
        $this->enabled = !empty($this->settings['enabled']) && in_array($this->settings['enabled'], ['yes', 1, true, '1'], true) ? 'yes' : 'no';
        $this->gateway = new Card();

        add_action('woocommerce_rest_checkout_process_payment_with_context', [$this, 'addPaymentIntent'], 9999, 2);
    }
    ...
    public function get_payment_method_data()
    {
        $data = [
            ...
            'capture_immediately' => in_array($this->settings['capture_immediately'], [true, 'yes'], true),
